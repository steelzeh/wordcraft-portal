...
import { EMessageDialogType, MessageDialogTypes } from '@/components/shared/MessageDialog/dialog-types.enum';
...
export default function MessageDialog({
  ...
  messageType: EMessageDialogType;
  didClickConfirm: () => void;
  didClickCancel?: () => void;
}) {
  const cancelButtonRef = useRef(null);
  const messageContent = MessageDialogTypes[messageType];
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                ...
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    ...
                    {messageContent.confirmButtonText}
                  </button>
                  {messageContent.showCancelButton ? (
                    <button
                      type="button"
                      ...
                      ref={cancelButtonRef}>
                      {messageContent.cancelButtonText}
                    </button>
                  ) : (
                    ''
                  )}
